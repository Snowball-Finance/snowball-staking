...
import { SNOWGLOBE_ABI, JOE_LP_ABI, PANGOLIN_LP_ABI } from 'libs/abis'
...
const GAUGE_INFO = async (pool) => {
  ...
  if (pool == '0xA42BE3dB9aff3aee48167b240bFEE5e1697e1281' || globe == '0xdE1A11C331a0E45B9BA8FeE04D4B51A745f1e4A4') {
    return getStablevaultInfo(pool)
  }
  else {
    return getSnowglobeInfo(pool)
  }
}

const getSnowglobeInfo = async (snowglobe) => {
  const SNOWGLOBE_CONTRACT = new ethers.Contract(snowglobe, SNOWGLOBE_ABI, library.getSigner())
  ...
  switch (g_symbol) {
    case 'sPGL':
      LP_TOKEN_CONTRACT = new ethers.Contract(lp_address, PANGOLIN_LP_ABI, library.getSigner())
    case 'sJLP':
      LP_TOKEN_CONTRACT = new ethers.Contract(lp_address, JOE_LP_ABI, library.getSigner())
  ...
  const TOKEN_0_CONTRACT = new ethers.Contract(token_0, ERC20_ABI, library.getSigner())
  const TOKEN_1_CONTRACT = new ethers.Contract(token_1, ERC20_ABI, library.getSigner())
  ...
  let token_name = `${lp_symbol} ${token_0_symbol}-${token_1_symbol}`
  let pool_name = `${token_0_symbol}-${token_1_symbol} pool`
...
//     }
//   },
const getStablevaultInfo = async (vault) => {
  const VAULT_CONTRACT = new ethers.Contract(vault, ERC20_ABI, library.getSigner())
  ...
  return {
    vault: {
      tokenName: symbol,
      poolName: `StableVault ${symbol} Pool`
    }
  }
